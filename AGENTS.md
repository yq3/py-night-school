...
- **hints 三级渐进**：第 1 级只给方向、第 2 级给形状、第 3 级才是接近完整的做法——答案只在最后一级（先例教训：初版第 1 级泄底被 review 打回）。「形状」= 伪代码/签名/提问式，**不含成行可抄的答案代码**（先例：Unit 2 复盘，讲义 Step 已展示同构实现，hints 不自觉复述成代码，五课系统性泄底）。自查法：把第 2 级抄进练习文件，能直接过验收即为泄底。
