...
> 昨晚 L4.1 的机制件落袋：`weighted_vote` 加权合成（共识靠算术不靠对话）、`apply_limits`
> 双闸门 clamp（只缩不放）、`DecisionCache` 缓存即审计。今晚共识从算术换成**辩论**：第二站
> TradingAgents——本单元唯一用标准图引擎的产品，L3.2–L3.4 熟的 StateGraph / 条件边 / reducer
> 第一次以「生产产品的全部拓扑」的规模出现；风控光谱上 L4.1 停在③处置层、本课回到②决策层。
> 明线照旧：金融辩论拓扑映射到**报销争议上诉**——CLM-2026-0004（发票校验未过被驳回）的
> 钱工对驳回提起上诉。
...
本课把它映射成**报销争议上诉**（课程调研的领域映射结论：报销争议/异常调查 =
...
`AgentBudgetExceeded`——同一条确定性护栏纪律：预算先验可算，不靠模型自觉）：
...
TradingAgents 的风控/审批「全部是 prompt 软约束，无任何硬性审批拦截代码」——放到
风控光谱上，这正是本课的位置：停在 ② 决策层（解析校验拦截），③④⑤ 一层都没有
（对照：L4.1 有 ③ 处置层 clamp，L4.3 直达 ④⑤）。仅有的两处「硬」保护都在**输出解析层**：

1. **哨兵值**：rating 解析失败返回 `REVIEW` 而不是捏造 `Hold`（`signal_processing.py`，
   产品 issue #1170 跟踪）——失败必须可见，绝不静默降级成「可执行的中性」；
2. **字段归一**：LLM 会往可选数值字段填占位串（"N/A"）、百分比（"15%"）、人写货币
   （"$1,234.50"）——`schemas.py#_coerce_optional_float` 把前两者归一为
...
### 2.8 新 Python 知识（给 Java 人的件）

- **嵌套 `TypedDict`**（`debate: DebateState`）≈ 内部静态类，但状态更新是**声明式整体
  替换**——节点返回 `{"debate": {...}}` 就顶掉整个嵌套值，不是「取出对象改一个字段
  set 回去」（§2.6 的手工回填纪律正是这个语义的代价面）；
- **纯计数器终止**：终止条件是整数自增比较（`count >= 2 * rounds`），手感对照 Java 的
  `AtomicInteger` 循环计数——本课把它当「可预算回环」的物理保证，不当语法点讲。
...
今晚三条机制全部直通毕业设计：条件边循环是 L5.1 重规划回环的原型——毕设静态图里
审批被打回后的重规划，就是这张 `申辩人 ⇄ 合规官` 的放大版，计数终止保证回环可预算、
绝不无限重试；REVIEW 哨兵是 L5.4 fail-closed 的输入端底线——今晚它管「解析不了的裁决
绝不捏造」，毕设把它扩成「不可解析即 DENY」的执行门纪律，同一句话的两种射程；图形状
签名是 L5.3 图版本绑定的先例——产品把轮次/分析师选择折进 checkpoint key，定义一变旧
执行态自动作废，毕设的事件溯源审计用同一个思想回答「结论在什么规则版本下产生」。
