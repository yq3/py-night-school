...
def aggregate(
    ...
    """按 FRAMEWORKS 的顺序把三路证据聚合成决策表行（参考答案）。"""
    rows: list[DecisionRow] = []
    for key in FRAMEWORKS:
        # 1) bench：同框架全 PASS 才 PASS；任一 FAIL 即 FAIL；无记录（mini-agent）为 "—"
        bench_hits = [r for r in bench_rows if r.framework == key]
        if not bench_hits:
            bench_cell = "—"
        elif all(r.status == "PASS" for r in bench_hits):
            bench_cell = "PASS"
        else:
            bench_cell = "FAIL"

        # 2) 定量：deps 取第一条非 None（同框架共用一个 lock）；手写总行数按序并列
        metric_hits = [m for m in metric_rows if m.framework == key]
        deps_value = next((m.deps for m in metric_hits if m.deps is not None), None)
        deps_cell = str(deps_value) if deps_value is not None else "缺"
        loc_values = [str(m.handwritten_loc) for m in metric_hits if m.handwritten_loc is not None]
        handwritten_cell = " / ".join(loc_values) if loc_values else "缺"

        # 3) 笔记：锁定性一句话小节的第一句；缺失/占位符 = MISSING_NOTE
        body = section_body(notes.get(key, ""), "锁定性一句话")
        line = first_meaningful_line(body)
        lock_line = line if line and PLACEHOLDER not in body else MISSING_NOTE

        rows.append(DecisionRow(FRAMEWORKS[key], bench_cell, deps_cell, handwritten_cell, lock_line))
    return rows
