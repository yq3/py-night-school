...
- **带着 mini-agent 上课**：每个框架课开场先对三问（Unit 2 出发前约定）——它的 runner
  对应 mini-agent 的哪几行？它的工具注册对应 `@tool` 注册表的哪些纪律？它的预算 /
  检查点参数对应双终止的哪一半？答不上来就回去翻 `unit2-mini-agent/milestone/`。
...
**工具**（`code/mock_tools.py`，框架课对版，不 import 任何框架）：
...
- 第 4 周：L3.7 + L3.8 + 里程碑（平台体感 + 决策表收口）。
...
对照笔记 + 自动生成的决策表**。选型工作台会以子进程逐课重跑共用验收脚本（四课的
contract 全绿才算数），并用脚本从四课的依赖清单与代码行数生成决策表数据页——
