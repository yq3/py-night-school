...
本课用「prompt 声明 + 解析校验 + 回喂重试」的通用方案；端点侧约束见 §2.5 的对照表。
"""
...
async def ask_structured(
    ...
    """问答并修复：直到拿到合法 PreapprovalDecision 或重试耗尽。
    ...
    for _attempt in range(1, attempts + 1):
        ...
        messages.append({"role": "assistant", "content": text})  # 模型产出永远先入史（L2.3 的纪律）
        try:
            return PreapprovalDecision.model_validate(extract_json(text)), messages
        except (ValueError, ValidationError) as exc:
            messages.append(feedback_message(exc))  # 修复指令回喂（坏产出已留在案底）
    raise StructuredOutputError(f"{attempts} 次尝试仍未得到合法决策 JSON（历史 {len(messages)} 条消息）")
