# 参考答案：ex2_repair（练习文件的完整解法——完成前别看）
...
async def ask_structured(model: ScriptedLite, question: str, attempts: int = 3) -> tuple[Decision, list[dict]]:
    ...
    for _attempt in range(1, attempts + 1):
        ...
        messages.append({"role": "assistant", "content": text})  # 先入史（好坏都入——审计）
        try:
            return Decision.model_validate(extract_json(text)), messages
        except (ValueError, ValidationError) as exc:
            messages.append({"role": "user", "content": f"上一次输出不合法（{exc}）。请只输出一个 JSON 对象。"})
    raise DecisionError(f"{attempts} 次尝试仍未得到合法决策 JSON")
