...
> 昨晚你的 `agent.py` 自己转起来了：ReAct 循环 + `ModelClient` 协议让真实端点与离线
> 剧本可互换，`max_turns` 预算兜住硬终止——它已经能干活了。但它交回的「最终回答」
> 还是一段散文：人看得懂，下游程序没法消费。今晚在循环出口装一道闸，教它把结论交成
> 带 schema 的正式文件——第四块肌肉（输出质量闸）。
...
- `find("{")` / `rfind("}")`：首尾大括号兜底——散文夹带（「决策如下：{...} 请查收」）
  也认。这是「宽进」（对模型的包裹格式宽容：代码块、前后闲话都剥掉，只认第一个 `{`
到最后一个 `}`——「严出」则指出口必有 schema 校验）的边界，JSON 里嵌套大括号
