...
> 昨晚你的工具注册表能被模型驱动了：`@tool` 函数定义即注册、schema 从参数模型生成、
> `run_tool` 把模型点名的参数一步解析校验后分发——但零件还各自为战，得你在旁边手动
> 接线。L2.1 的协议层与 L2.2 的工具层都已在手，今晚装上会自己转的循环：
> 一个真正的 ReAct 循环。第三块肌肉（循环骨架）。

## 1. 本课目标

前两课的零件今晚合体——`messages` 协议（L2.1）+ 工具注册表（L2.2），再加上本课新造的
客户端抽象 `ModelClient`，装成一个 **真正的 agent 循环**。完成后你能：
...
你已经见过它一次：昨晚 demo_registry 里那个「请求 → 选工具 → 执行 → 回喂」的
while 就是它的雏形（当时只缺轮数预算）；今晚把它抽成类、配齐终止与预算：
...
就写一条 content 脚本。零 key、零网络，验收照样确定——这是 L2.1 mock 端点思路的
再进一步：连模型本身都换成了剧本。
...
（硬终止）、`AgentResult`（结果三件套：`final_text` 最终回答 / `messages` 完整历史 /
