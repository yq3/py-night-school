...
PEP8 的纪律：lambda 到条件表达式（`x if cond else y`）就是复杂度上限，
...
「模型选工具 → 摊开参数 → 调 Python 函数」的那一行；闭包与非local 则是 L1.8 重试装饰器
（L1.5 装饰器 = 闭包的亲儿子）的一半地基。函数当值用，是从今晚开始到毕业一路的底层通货。
