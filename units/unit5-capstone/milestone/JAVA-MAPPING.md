...
>
> 学员任务（毕业定稿）：下表有 **4 行的 Java 对应物留了 TODO 占位**——对照本地克隆
> （`~/develop/opensource/` 的 langgraph4j / spring-ai-alibaba）核实后补全它们；这是毕业
> 判据的第二块——占位清零才算定稿。结构校验在 `tests/test_mapping_meta.py`（主表 ≥13 行 +
> 四列非空 + 「需自建」行保留 + 占位清零）。参考对照版在 `solution/JAVA-MAPPING.md`
> （行尾 golden answer 诚实降级：开放设计不硬造判分——先自己写再对，差异处才是认知增量）。
...
| fail-closed 执行门（三态检查链） | `gate.check_intent`：纯函数七查固定顺序、`GateVerdict{ALLOW/DENY/PAUSE_FOR_REAUTH}`、不可解析即 DENY、时钟/账本注入 | TODO(毕业)：对照讲义 §2 与 L4.3 读过的 enforcement 原型补全（提示：Java 侧没有框架级的「门」，这是**你自己写的一个类**——想清楚它的形状：静态方法？enum？异常还是返回值？） | 提示（补全时想清楚再写）：①L4.3 读过的 Vibe-Trading `check_mandate` 就是 Java 域原型——纯函数 + 常量顺序 + BreachEvent；②Java 有 checked exception 的「要么 catch 要么 throws」，反而容易写出「catch 后返回 null」——**门的返回值里没有 null 的位置**（连 ALLOW 都是一等值）；③顺序即语义：私有检查方法的调用顺序就是审计契约 |
| 缓存即审计（LLM 决策留原话） | `audit_cache.DecisionCache`：`llm_decisions(prompt_hash PRIMARY KEY, prompt, response)` + `AuditedModel`（Runnable 装饰） | TODO(毕业)：补全（提示：JPA Entity + 主键点查 + upsert 是最小形状；装饰层对标 Spring 的 `Advisor`/代理——想清楚「换的是芯不是壳」在 Java 怎么表达） | 提示（补全时想清楚再写）：①canonical 序列化要在 Java 重写——langchain4j 消息对象的 type 名与 langchain 不同（`user`/`assistant` 两侧方言），**哈希口径要用测试锁死**（同 prompt 两侧必须同 hash，否则迁移后缓存全 miss 且审计对不上账）；②INSERT OR REPLACE ≈ save()+@Id，但「最新原话覆盖旧原话」的审计语义要想清楚 |
...
| 拒绝回环（人审反馈回喂） | `submit --(reject)--> drafter` 回边 + `approval_rejects` 合并账本 + `MAX_APPROVAL_LOOPS` 封顶 + 双哨兵分码 | TODO(毕业)：补全（提示：图侧是条件边回环 + 计数状态键；Java 侧对应 langgraph4j/saa 的哪个 API 能改状态再回环？核实 `CompiledGraph.updateState`） | 提示（补全时想清楚再写）：langgraph4j `CompiledGraph.updateState(config, values, asNode)`（CompiledGraph.java，saa 同名）≈「以某节点身份补写状态」——反馈消息回喂用它；**封顶计数器在哪算**（路由函数里数 vs 节点里数）决定哨兵分码能不能写对 |
