...
def plan_gate(state: ReplanState) -> dict:
    """校验门（本体复用讲义 plan.validate_plan）：拒绝分支把原因写进合并语义的状态键。"""
    outcome = validate_plan(state["messages"][-1].content)
    if isinstance(outcome, PlanRejection):
        return {
            "plan_rejections": [outcome],  # 装一个元素的列表——operator.add reducer 每次收到一个列表
            "events": [f"plan.rejected:{outcome.reason_code}"],  # 审计事件带原因码
        }
    return {"plan": outcome, "events": ["plan.approved"]}


def route_after_gate(state: ReplanState) -> Literal["executor", "planner", "escalate"]:
    if state.get("plan") is not None:  # NotRequired 键缺席时 get 返回 None
        return "executor"
    if len(state.get("plan_rejections") or []) <= MAX_REPLANS:  # 第 1/2 次拒绝回去；第 3 次 replans 已用满
        return "planner"
    return "escalate"
