...
def validate_plan(text: str) -> Plan | PlanRejection:
    ...
    # TODO(ex1): 先 json.loads（不是 JSON 时 return _not_json_rejection(exc)——分支已给），然后——问：
    #   1) 用哪个 Pydantic 入口对解析产物做「带判别联合」的校验？失败抛什么、又能从 e.errors()[0] 拿到什么？
    #   2) 首错的 loc 与 msg 怎么拼成 detail（出错位置要能指到具体步骤）？
    #   3) 校验通过后，为什么还要对每步的 step.tool 再查一遍 TOOL_ALLOWLIST？不在时返回什么？
    raise NotImplementedError("TODO(ex1): 补全校验门")
