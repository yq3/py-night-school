...
def make_execute(policy: Policy, ledger: MemoryLedger, amount_cents: int):
    ...
    async def execute(state: MiniState) -> dict:
        ...
        verdict = check_intent(  # noqa: F841 —— 骨架纪律：TODO 填完后被三出口消费（预置防未使用告警）
            policy,
            intent,
            _record_of(approval_raw),
            LedgerView(today="2026-09-16", payments=ledger.payments_for()),
        )
        # TODO(ex2): 三出口——ALLOW（记账+paid_cents+两条事件） / DENY（gate_reject+gate.denied 事件）
        #   / PAUSE（gate_reject+gate.paused 事件）；事件流进 "events"（合并语义键）
        raise NotImplementedError("TODO(ex2): execute 三出口")
...
def route_after_execute(state: MiniState) -> str:
    """执行门后的两分支（你的 TODO）：付了 → END；没付 → escalate。"""
    # TODO(ex2): 读哪个键区分「付了/没付」？返回值里 END 是 langgraph 的常量
    raise NotImplementedError("TODO(ex2): route_after_execute")
