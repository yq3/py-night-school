# 练习 3 参考答案（solution/ 覆盖 exercises/ 后跑验收全绿；与骨架的差异只在 TODO 区）
    ...
    for section, rows in tables.items():
        for lineno, row in enumerate(rows, start=1):
            if len(row) != len(COLUMNS):
                problems.append(f"{section} 第 {lineno} 行缺列：{len(row)}/{len(COLUMNS)}")
            elif not row[2]:
                problems.append(f"{section} 第 {lineno} 行 Java 对应物为空")
            elif TODO_MARK in row[2]:
                problems.append(f"{section} 第 {lineno} 行未填（TODO 未补全）")
    return problems
